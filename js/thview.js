...
ThView.prototype.rotateCamera = function(x, y) {
    ...
    console.log(this.oldPosition.x, pos.x, this.pan);
...
ThView.prototype.animateTo = function(_pan, _tilt){
    _tilt = _tilt || 0;
    _pan = _pan || 0;
    _pan = _pan * (Math.PI / 180);
    this.pan -=  _pan;
    this.tilt = _tilt;
    console.log("pan", this.pan * 180 / Math.PI, "tilt", this.tilt * 180 / Math.PI)
    this.cameraDir.x = Math.sin(this.pan) * Math.cos(this.tilt);
    this.cameraDir.z = Math.cos(this.pan) * Math.cos(this.tilt);
    this.cameraDir.y = Math.sin(this.tilt);
    this.camera.lookAt(this.cameraDir);
...
ThView.prototype.forceanimateTo = function(_pan, _tilt){
    _tilt = _tilt || 0;
    _pan = _pan || 0;
    _pan = _pan * (Math.PI / 180);
    this.pan =  _pan;
